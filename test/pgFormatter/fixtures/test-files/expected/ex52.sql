...
CREATE FUNCTION
ADD (integer, integer) RETURNS integer LANGUAGE sql IMMUTABLE STRICT AS $_$
        SELECT
            $1 + $2;
...
CREATE TABLE nv_child_2009 (
    CHECK (
        d BETWEEN '2009-01-01'::date AND '2009-12-31'::date)) inherits (nv_parent);

CREATE TABLE part1 (
    a int NOT NULL CHECK (a = 1),
    b int NOT NULL CHECK (
        b >= 1
        AND b <= 10));
...
OR
UPDATE ON alterlock FOR each ROW
