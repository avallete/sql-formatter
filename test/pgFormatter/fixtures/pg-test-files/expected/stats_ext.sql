...
CREATE STATISTICS tst (unrecognized) ON relname,
relnatts
FROM
    pg_class;
...
CREATE TABLE tststats.pt (a int, b int, c text)
PARTITION BY
    RANGE (a, b);

CREATE TABLE tststats.pt1 PARTITION OF tststats.pt FOR
VALUES
FROM
    (-10, -10) TO (10, 10);
...
INSERT INTO
    functional_dependencies (a, b, c, filler1)
SELECT
    mod(i, 23),
    mod(i, 29),
    mod(i, 31),
...
-- create statistics
CREATE STATISTICS func_deps_stat (dependencies) ON a,
b,
c
FROM
...
-- create statistics
CREATE STATISTICS mcv_lists_stats (mcv) ON a,
b,
c
FROM
    mcv_lists;
...
SELECT (
        CASE
            WHEN mod(i, 100) = 1 THEN
                NULL
            ELSE
                mod(i, 100)
        END), (
        CASE
            WHEN mod(i, 50) = 1 THEN
                NULL
            ELSE
                mod(i, 50)
        END), (
        CASE
            WHEN mod(i, 25) = 1 THEN
                NULL
            ELSE
                mod(i, 25)
        END),
...
CREATE STATISTICS mcv_lists_arrays_stats (mcv) ON a,
b,
c
FROM
    mcv_lists_arrays;
...
CREATE STATISTICS mcv_lists_bool_stats (mcv) ON a,
b,
c
FROM
    mcv_lists_bool;
