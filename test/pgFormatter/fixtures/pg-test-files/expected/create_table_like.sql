...
INSERT INTO
    inhe
VALUES
    ('ee-col1', 'ee-col2', DEFAULT, 'ee-col4');
...
/* Throw error */
CREATE TABLE inhf (
    LIKE inhx INCLUDING DEFAULTS INCLUDING CONSTRAINTS);
...
CREATE TABLE ctlt1_inh (
    LIKE ctlt1 INCLUDING CONSTRAINTS INCLUDING COMMENTS) INHERITS (ctlt1);
...
FROM
    ...
    pg_constraint c
WHERE
    classoid = 'pg_constraint'::regclass
    AND objoid = c.oid
    AND c.conrelid = 'ctlt1_inh'::regclass;

CREATE TABLE ctlt13_inh () INHERITS (ctlt1, ctlt3);

\d+ ctlt13_inh
CREATE TABLE ctlt13_like (
    LIKE ctlt3 INCLUDING CONSTRAINTS INCLUDING COMMENTS INCLUDING STORAGE) INHERITS (ctlt1);
...
WHERE
    classoid = 'pg_constraint'::regclass
    AND objoid = c.oid
    AND c.conrelid = 'ctlt13_like'::regclass;
...
FROM
    ...
    pg_class c
WHERE
    classoid = 'pg_class'::regclass
    AND objoid = i.indexrelid
    AND c.oid = i.indexrelid
    AND i.indrelid = 'ctlt_all'::regclass
ORDER BY
    c.relname,
    objsubid;
...
FROM
    ...
    pg_statistic_ext s
WHERE
    classoid = 'pg_statistic_ext'::regclass
    AND objoid = s.oid
    AND s.stxrelid = 'ctlt_all'::regclass
ORDER BY
    s.stxname,
    objsubid;
