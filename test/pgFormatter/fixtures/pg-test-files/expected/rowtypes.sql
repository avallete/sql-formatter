...
SELECT
    (1.1, 2.2)::complex,
    ROW ((3.3, 4.4), (5.5, NULL))::quad;

SELECT
    ROW ('Joe', 'Blow')::fullname,
    '(Joe,Blow)'::fullname;
...
INSERT INTO
    pp
VALUES
    (repeat('abcdefghijkl', 100000));

INSERT INTO
    people
SELECT
    ('Jim', f1, NULL)::fullname,
    current_date
FROM
    pp;
...
SELECT
    ROW (1, 2) < ROW (1, 3) AS TRUE;

SELECT
    ROW (1, 2) < ROW (1, 1) AS FALSE;

SELECT
    ROW (1, 2) < ROW (1, NULL) AS NULL;

SELECT
    ROW (1, 2, 3) < ROW (1, 3, NULL) AS TRUE;

-- the NULL is not examined
SELECT
    ROW (11, 'ABC') < ROW (11, 'DEF') AS TRUE;

SELECT
    ROW (11, 'ABC') > ROW (11, 'DEF') AS FALSE;

SELECT
    ROW (12, 'ABC') > ROW (11, 'DEF') AS TRUE;

-- = and <> have different NULL-behavior than < etc
SELECT
    ROW (1, 2, 3) < ROW (1, NULL, 4) AS NULL;

SELECT
    ROW (1, 2, 3) = ROW (1, NULL, 4) AS FALSE;

SELECT
    ROW (1, 2, 3) <> ROW (1, NULL, 4) AS TRUE;

-- We allow operators beyond the six standard ones, if they have btree
-- operator classes.
SELECT
    ROW ('ABC', 'DEF') ~<=~ ROW ('DEF', 'ABC') AS TRUE;

SELECT
    ROW ('ABC', 'DEF') ~>=~ ROW ('DEF', 'ABC') AS FALSE;

SELECT
    ROW ('ABC', 'DEF') ~~ ROW ('DEF', 'ABC') AS fail;

-- Comparisons of ROW() expressions can cope with some type mismatches
SELECT
    ROW (1, 2) = ROW (1, 2::int8);

SELECT
    ROW (1, 2) IN (ROW (3, 4), ROW (1, 2));

SELECT
    ROW (1, 2) IN (ROW (3, 4), ROW (1, 2::int8));
...
SELECT
    ROW (1, 1.1) = ANY (ARRAY[ROW (7, 7.7), ROW (1, 1.1), ROW (0, 0.0)]);

SELECT
    ROW (1, 1.1) = ANY (ARRAY[ROW (7, 7.7), ROW (1, 1.0), ROW (0, 0.0)]);
...
-- all true
SELECT
    ROW (1, 2)::testtype1 < ROW (1, 3)::testtype1;

SELECT
    ROW (1, 2)::testtype1 <= ROW (1, 3)::testtype1;

SELECT
    ROW (1, 2)::testtype1 = ROW (1, 2)::testtype1;

SELECT
    ROW (1, 2)::testtype1 <> ROW (1, 3)::testtype1;

SELECT
    ROW (1, 3)::testtype1 >= ROW (1, 2)::testtype1;
...
SELECT
    ROW (1, 2)::testtype1 < ROW (1, 'abc')::testtype3;
...
SELECT
    ROW (1, 2)::testtype1 < ROW (1)::testtype5;
...
SELECT
    ROW (1, '(1,2)')::testtype6 < ROW (1, '(1,3)')::testtype6;
...
SELECT
    ROW (1, TRUE)::testtype2 * < ROW (2, TRUE)::testtype2;

SELECT
    ROW (-2, TRUE)::testtype2 * < ROW (-1, TRUE)::testtype2;

SELECT
    ROW (0, FALSE)::testtype2 * < ROW (0, TRUE)::testtype2;
...
-- variable length
SELECT
    ROW (1, 'abc')::testtype3 * < ROW (1, 'abd')::testtype3;

SELECT
    ROW (1, 'abc')::testtype3 * < ROW (1, 'abcd')::testtype3;
...
-- by ref, fixed length
SELECT
    ROW (1, '(1,2)')::testtype4 * < ROW (1, '(1,3)')::testtype4;
...
FROM
    unnest (
        ARRAY [
            ...
            ROW (2, ROW (3), ARRAY[ROW (4)], ROW (ROW (5)))]) AS q (a int, b record, c record[], d record);
