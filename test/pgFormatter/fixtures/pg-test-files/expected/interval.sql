...
SELECT
    INTERVAL '-1 days +02:03' AS "22 hours ago...";

SELECT
    INTERVAL '1.5 weeks' AS "Ten days twelve hours";

SELECT
    INTERVAL '1.5 months' AS "One month 15 days";
...
FROM
    INTERVAL_TBL
WHERE
    INTERVAL_TBL.f1 <> interval '@ 10 days';
...
FROM
    INTERVAL_TBL
WHERE
    INTERVAL_TBL.f1 < interval '@ 1 day';
...
FROM
    INTERVAL_TBL
WHERE
    INTERVAL_TBL.f1 = interval '@ 34 years';
...
FROM
    INTERVAL_TBL
WHERE
    INTERVAL_TBL.f1 >= interval '@ 1 month';
...
FROM
    INTERVAL_TBL
WHERE
    INTERVAL_TBL.f1 > interval '@ 3 seconds ago';
...
SELECT
    '4 millenniums 5 centuries 4 decades 1 year 4 months 4 days 17 minutes 31 seconds'::interval;
...
SELECT
    '3 days 5 milliseconds'::interval;

SELECT
    '1 second 2 seconds'::interval;

-- error
SELECT
    '10 milliseconds 20 milliseconds'::interval;

-- error
SELECT
    '5.5 seconds 3 milliseconds'::interval;

-- error
SELECT
    '1:20:05 5 microseconds'::interval;
...
SELECT
    interval '1 year 2 mons 3 days 04:05:06.699999';
...
SELECT
    make_interval(months := 'NaN'::float::int);

SELECT
    make_interval(secs := 'inf');

SELECT
    make_interval(secs := 'NaN');
