...
CREATE OPERATOR CLASS box_ops DEFAULT FOR TYPE box USING gist2 AS OPERATOR 1 <<,
OPERATOR 2 &<,
OPERATOR 3 &&,
OPERATOR 4 &>,
OPERATOR 5 >>,
...
SET
    default_table_access_method = 'I do not exist AM';

-- prevent setting it to an index AM
SET
    default_table_access_method = 'btree';
...
-- CREATE TABLE AS
CREATE TABLE tableam_tblas_heap2 USING heap2 AS
SELECT
    *
FROM
    tableam_tbl_heap2;
...
SELECT INTO tableam_tblselectinto_heap2 USING heap2
FROM
    tableam_tbl_heap2;
...
CREATE TABLE tableam_parted_heap2 (a text, b int)
PARTITION BY
    list (a) USING heap2;
...
SET
    default_table_access_method = 'heap2';
...
FROM
    ...
    pg_am
WHERE
    pg_depend.refclassid = 'pg_am'::regclass
...
SET
    LOCAL default_table_access_method = 'heap2';

-- following tests should all respect the default AM
CREATE TABLE tableam_tbl_heapx (f1 int);

CREATE TABLE tableam_tblas_heapx AS
SELECT
    *
FROM
    tableam_tbl_heapx;

SELECT INTO tableam_tblselectinto_heapx
FROM
    tableam_tbl_heapx;
...
CREATE TABLE tableam_parted_heapx (a text, b int)
PARTITION BY
    list (a);

CREATE TABLE tableam_parted_1_heapx PARTITION OF tableam_parted_heapx FOR
VALUES
    IN ('a', 'b');

-- but an explicitly set AM overrides it
CREATE TABLE tableam_parted_2_heapx PARTITION OF tableam_parted_heapx FOR
VALUES
    IN ('c', 'd') USING heap;

-- sequences, views and foreign servers shouldn't have an AM
CREATE VIEW tableam_view_heapx AS
SELECT
    *
FROM
    tableam_tbl_heapx;
