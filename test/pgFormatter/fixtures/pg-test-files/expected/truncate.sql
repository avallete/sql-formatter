...
TRUNCATE ON trunc_trigger_test FOR EACH STATEMENT
EXECUTE PROCEDURE trunctrigger ('before trigger truncate');

SELECT
    count(*) AS "Row count in test table"
FROM
    trunc_trigger_test;
...
CREATE SEQUENCE truncate_a_id1 START
WITH
    33;
...
CREATE TABLE truncparted1 PARTITION OF truncparted FOR
VALUES
    IN (1);
...
CREATE FUNCTION tp_chk_data (
    OUT pktb regclass,
    OUT pkval int,
    OUT fktb regclass,
    OUT fkval int) RETURNS SETOF record LANGUAGE plpgsql AS $$
  BEGIN
    RETURN QUERY SELECT
...
CREATE TABLE truncpart_1 PARTITION OF truncpart FOR
VALUES
FROM
    (0) TO (100);

CREATE TABLE truncpart_2 PARTITION OF truncpart FOR
VALUES
FROM
    (100) TO (200)
PARTITION BY
...
CREATE TABLE truncpart_2_1 PARTITION OF truncpart_2 FOR
VALUES
FROM
    (100) TO (150);
...
DROP TABLE truncprim,
truncpart;

DROP FUNCTION tp_ins_data (),
tp_chk_data ();