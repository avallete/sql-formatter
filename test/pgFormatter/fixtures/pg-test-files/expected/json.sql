...
FROM
    json_populate_record (
        ROW ('x', 3, '2012-12-31 15:30:56')::jpop,
        '{"a":"blurfl","x":43.2}') q;
...
SELECT
    *
FROM
    json_populate_recordset(ROW (0::int), '[{"a":"1","b":"2"},{"a":"3"}]') q (a text, b text);
...
SELECT
    json_build_object (
        ...
        json_object_agg (
            serial_num,
            json_build_object('name', name, 'type', type)))
FROM
    foo;
...
SELECT
    *
FROM
    json_to_record('{"a":1,"b":"foo","c":"bar"}') AS x (a int, b text, d text);
...
FROM
    json_to_recordset (
        '[{"a":1,"b":{"d":"foo"},"c":true},{"a":2,"c":false,"b":{"d":"bar"}}]') AS x (a int, b json, c boolean);
...
FROM
    json_to_record (
        '{"a":1, "b":{"c":16, "d":2}, "x":8, "ca": ["1 2", 3], "ia": [[1,2],[3,4]], "r": {"a": "aaa", "b": 123}}'::json) AS t (
        a int,
        b json,
        c text,
        x int,
...
SELECT
    *,
    c IS NULL AS c_is_null
FROM
    json_to_recordset('[{"a":1, "b":{"c":16, "d":2}, "x":8}]'::json) AS t (a int, b json, c text, x int);
...
SELECT
    to_tsvector (
        '{"a": "aaa bbb ddd ccc", "b": ["eee fff ggg"], "c": {"d": "hhh iii"}}'::json);
...
SELECT
    to_tsvector (
        'english',
        '{"a": "aaa in bbb ddd ccc", "b": ["the eee fff ggg"], "c": {"d": "hhh. iii"}}'::json);

-- json to tsvector with numeric values
SELECT
    to_tsvector (
        'english',
        '{"a": "aaa in bbb ddd ccc", "b": 123, "c": 456}'::json);
...
SELECT
    ts_headline (
        '{"a": "aaa bbb", "b": {"c": "ccc ddd fff", "c1": "ccc1 ddd1"}, "d": ["ggg hhh", "iii jjj"]}'::json,
        tsquery ('bbb & ddd & hhh'));

SELECT
    ts_headline (
        'english',
        '{"a": "aaa bbb", "b": {"c": "ccc ddd fff"}, "d": ["ggg hhh", "iii jjj"]}'::json,
        tsquery ('bbb & ddd & hhh'));
