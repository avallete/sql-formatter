...
GRANT
EXECUTE ON FUNCTION f_leak (text) TO public;
...
CREATE POLICY p1 ON document AS PERMISSIVE USING (
    dlevel <= (
        SELECT
            seclv
        FROM
...
CREATE POLICY p1 ON document AS UGLY USING (
    dlevel <= (
        SELECT
            seclv
        FROM
...
-- to p1r first
CREATE POLICY p2r ON document AS RESTRICTIVE TO regress_rls_dave USING (
    cid <> 44
    AND cid < 50);
...
SELECT
    *
FROM
    document TABLESAMPLE BERNOULLI (50) REPEATABLE (0)
WHERE
    f_leak (dtitle)
ORDER BY
    did;
...
CREATE POLICY p2 ON category USING (
    CASE
        WHEN current_user = 'regress_rls_bob' THEN
            cid IN (11, 33)
...
CREATE TABLE part_document_fiction PARTITION OF part_document FOR
VALUES
FROM
    (11) TO (12);

CREATE TABLE part_document_satire PARTITION OF part_document FOR
VALUES
FROM
    (55) TO (56);

CREATE TABLE part_document_nonfiction PARTITION OF part_document FOR
VALUES
FROM
    (99) TO (100);
...
-- user's security level must be higher than or equal to document's
CREATE POLICY pp1 ON part_document AS PERMISSIVE USING (
    dlevel <= (
        SELECT
            seclv
        FROM
...
CREATE POLICY pp3 ON part_document AS RESTRICTIVE USING ( (
        SELECT
            dlevel <= seclv
        FROM
            uaccount
...
CREATE POLICY d1 ON dependent FOR ALL TO PUBLIC USING (
    x = (
        SELECT
            d.x
        FROM
...
CREATE POLICY r1 ON rec1 USING (
    x = (
        SELECT
            r.x
        FROM
...
CREATE POLICY r2 ON rec2 USING (
    a = (
        SELECT
            x
        FROM
...
CREATE POLICY p3 ON s1 FOR INSERT
WITH
    CHECK (
        a = (
            SELECT
...
CREATE POLICY p1 ON document FOR
SELECT
    USING (TRUE);

CREATE POLICY p2 ON document FOR INSERT
WITH
    CHECK (dauthor = current_user);
...
VALUES (
        ...
        1,
        'regress_rls_bob',
        'Some novel, replaces sci-fi') -- takes UPDATE path
ON CONFLICT (did) DO UPDATE
SET
    dtitle = EXCLUDED.dtitle;
...
-- tests)
--
CREATE POLICY p3_with_all ON document FOR ALL USING (
    cid = (
        SELECT
            cid
        FROM
...
GRANT
SELECT
    ON rls_view TO regress_rls_bob;
...
GRANT
SELECT
    ON rls_view TO regress_rls_alice;
...
GRANT
SELECT
    ON rls_view TO regress_rls_carol;
...
CREATE POLICY p1 ON x1 FOR
SELECT
    USING (a % 2 = 0);

CREATE POLICY p2 ON x1 FOR INSERT
WITH
    CHECK (a % 2 = 1);
...
GRANT ALL ON y1,
y2 TO regress_rls_bob;

CREATE POLICY p1 ON y1 FOR ALL USING (a % 2 = 0);

CREATE POLICY p2 ON y1 FOR
SELECT
    USING (a > 2);

CREATE POLICY p1 ON y1 FOR
SELECT
    USING (a % 2 = 1);
...
GRANT ALL ON copy_t TO regress_rls_bob,
...
GRANT ALL ON copy_rel_to TO regress_rls_bob,
regress_rls_exempt_user;
...
CREATE POLICY p1 ON current_check FOR
SELECT
    USING (currentid % 2 = 0);

CREATE POLICY p2 ON current_check FOR DELETE USING (
    currentid = 4
...
GRANT
SELECT
    ON coll_t TO regress_rls_alice;
...
CREATE TABLE tbl1 (c) AS
VALUES
    ('bar'::text);

GRANT
SELECT
    ON TABLE tbl1 TO regress_rls_eve;

CREATE POLICY P ON tbl1 TO regress_rls_eve,
regress_rls_frank USING (TRUE);
...
WHERE
    classid = 'pg_policy'::regclass
    AND refobjid IN (
        'regress_rls_eve'::regrole,
        'regress_rls_frank'::regrole);
...
GRANT ALL ON r1,
r2 TO regress_rls_bob;

CREATE POLICY p1 ON r1 USING (TRUE);

ALTER TABLE r1 ENABLE ROW LEVEL SECURITY;

CREATE POLICY p1 ON r2 FOR
SELECT
    USING (TRUE);

CREATE POLICY p2 ON r2 FOR INSERT
WITH
    CHECK (FALSE);
...
CREATE POLICY p3 ON r1 FOR INSERT
WITH
    CHECK (TRUE);
...
VALUES
    (10)
ON CONFLICT ON CONSTRAINT r1_pkey DO UPDATE
SET
    a = 30;
...
CREATE POLICY dep_p1 ON dep1 TO regress_rls_bob USING (
    c1 > (
        SELECT
            max(dep2.c1)
        FROM
...
ALTER POLICY dep_p1 ON dep1 TO regress_rls_bob,
regress_rls_carol;
...
regress_rls_dob_role2 USING (TRUE);
...
-- should succeed
CREATE POLICY p1 ON dob_t2 TO regress_rls_dob_role1,
regress_rls_dob_role2 USING (TRUE);
...
CREATE POLICY p1 ON rls_tbl USING (
    EXISTS (
        SELECT
            1
        FROM
...
GRANT
SELECT
    ON ref_tbl TO regress_rls_bob;

GRANT
SELECT
    ON rls_tbl TO regress_rls_bob;
...
CREATE POLICY p2 ON rls_tbl FOR
SELECT
    USING (c1 <= 3);
...
CREATE POLICY p1 ON rls_tbl_force USING (c1 = 5)
WITH
    CHECK (c1 < 5);

CREATE POLICY p2 ON rls_tbl_force FOR
SELECT
    USING (c1 = 8);
