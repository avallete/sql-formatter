...
WITH RECURSIVE
    subdepartment (level, id, parent_department, name) AS (
        -- non recursive term
        SELECT
            1,
            *
...
SELECT
    pg_get_viewdef('vsubdepartment'::regclass);
...
FROM
    ...
    JOIN t AS t2 ON (
        t1.path[1] = t2.path[1]
        AND array_upper(t1.path, 1) = 1
        AND array_upper(t2.path, 1) > 1)
ORDER BY
    t1.id,
    t2.id;
...
WITH RECURSIVE
    search_graph (f, t, label, path, cycle) AS (
        SELECT
            *,
            ARRAY[ROW (g.f, g.t)],
            FALSE
...
WITH RECURSIVE
    ...
    iter (id_key, row_type, link) AS (
        ...
        UNION ALL (
            WITH
                remaining (id_key, row_type, link, min) AS (
                    SELECT
                        tab.id_key,
                        'true'::text,
                        iter.link,
                    ...
                    FROM
                        tab
                        INNER JOIN iter USING (link)
                    WHERE
                        tab.id_key > iter.id_key),
                first_remaining AS (
                    SELECT
                        id_key,
                        row_type,
...
AFTER INSERT ON y FOR EACH ROW
EXECUTE PROCEDURE y_trigger ();
...
CREATE TRIGGER y_trig
AFTER INSERT ON y FOR EACH STATEMENT
EXECUTE PROCEDURE y_trigger ();
...
CREATE RULE y_rule AS ON INSERT TO y
WHERE
    a = 0 DO INSTEAD
DELETE FROM y;
