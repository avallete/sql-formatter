...
CREATE MATERIALIZED VIEW mvtest_tmm AS
SELECT
    sum(totamt) AS grandtot
FROM
    mvtest_tm;
...
CREATE UNIQUE INDEX mvtest_tvmm_pred ON mvtest_tvmm (grandtot)
WHERE
    grandtot < 0;

CREATE VIEW mvtest_tvv AS
SELECT
    sum(totamt) AS grandtot
FROM
    mvtest_tv;
...
CREATE VIEW mvtest_tvvmv AS
SELECT
    *
FROM
    mvtest_tvvm;

CREATE MATERIALIZED VIEW mvtest_bb AS
SELECT
    *
FROM
    mvtest_tvvmv;
...
CREATE VIEW mvtest_vt1 AS
SELECT
    1 moo;

CREATE VIEW mvtest_vt2 AS
SELECT
    moo,
    2 * moo
FROM
...
\d+ mvtest_vt2
CREATE MATERIALIZED VIEW mv_test2 AS
SELECT
    moo,
    2 * moo
FROM
...
\d+ mv_test2
CREATE MATERIALIZED VIEW mv_test3 AS
SELECT
    *
FROM
    mv_test2
...
-- error
CREATE MATERIALIZED VIEW mvtest_mv_v (ii, jj) AS
SELECT
    i,
    j
FROM
...
-- ok
CREATE MATERIALIZED VIEW mvtest_mv_v_2 (ii) AS
SELECT
    i,
    j
FROM
...
-- ok
CREATE MATERIALIZED VIEW mvtest_mv_v_3 (ii, jj, kk) AS
SELECT
    i,
    j
FROM
...
CREATE TABLE mvtest_foo_data AS
SELECT
    i,
    md5(random()::text)
FROM
...
CREATE MATERIALIZED VIEW mvtest_mv_foo AS
SELECT
    *
FROM
    mvtest_foo_data;
