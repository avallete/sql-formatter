...
INSERT INTO
    brintest (inetcol, cidrcol, int4rangecol)
SELECT
...
CREATE TABLE brin_summarize (value int)
WITH
    (fillfactor = 10, autovacuum_enabled = FALSE);

CREATE INDEX brin_summarize_idx ON brin_summarize USING brin (value)
WITH
    (pages_per_range = 2);
...
CREATE INDEX brin_test_a_idx ON brin_test USING brin (a)
WITH
    (pages_per_range = 2);

CREATE INDEX brin_test_b_idx ON brin_test USING brin (b)
WITH
    (pages_per_range = 2);
