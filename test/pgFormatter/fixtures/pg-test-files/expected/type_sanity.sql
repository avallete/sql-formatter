...
FROM
    pg_type AS p1
WHERE
    p1.typtype NOT IN ('c', 'd', 'p')
    AND p1.typname NOT LIKE E'\\_%'
    AND NOT EXISTS (
        SELECT
            1
        FROM
            pg_type AS p2
        WHERE
            p2.typname = ('_' || p1.typname)::name
            AND p2.typelem = p1.oid
            AND p1.typarray = p2.oid);
...
FROM
    pg_type AS p1
    LEFT JOIN pg_type AS p2 ON p1.typbasetype = p2.oid
WHERE
    p1.typtype = 'd'
    AND p1.typoutput IS DISTINCT FROM p2.typoutput;
...
WHERE
    p1.typinput = p2.oid
    AND p1.typreceive = p3.oid
    AND p2.pronargs != p3.pronargs;
...
WHERE
    p1.typtype = 'd'
    AND p1.typsend IS DISTINCT FROM p2.typsend;
...
WHERE
    p1.typarray = p2.oid
    AND NOT (p1.typdelim = p2.typdelim);
...
FROM
    pg_type d
    JOIN pg_type t ON d.typbasetype = t.oid
WHERE
...
WHERE
    t.typbasetype = 0
    AND (t.typanalyze = 'range_typanalyze'::regproc) != (r.rngtypid IS NOT NULL);
...
FROM
    pg_range p1
    JOIN pg_proc p ON p.oid = p1.rngcanonical
WHERE
    pronargs != 1
    OR proargtypes[0] != rngtypid
    OR prorettype != rngtypid;
...
FROM
    pg_range p1
    JOIN pg_proc p ON p.oid = p1.rngsubdiff
WHERE
    pronargs != 2
