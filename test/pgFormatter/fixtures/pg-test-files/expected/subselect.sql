...
SELECT
    *, (
        ...
        SELECT
            CASE
                WHEN ord.ordercanceled THEN
                    'Canceled'
                ELSE (
                        SELECT
                            CASE
                                WHEN ord.po_ref = 1 THEN (
                                        SELECT
                                            CASE
                                                WHEN ord.approver_ref = 1 THEN
                                                    '---'
...
INSERT INTO
    shipped_view (ordnum, partnum, value)
VALUES (
        0,
        1, (
            SELECT
...
--
CREATE FUNCTION tattle (x int, y int) returns bool volatile language plpgsql AS $$
begin
