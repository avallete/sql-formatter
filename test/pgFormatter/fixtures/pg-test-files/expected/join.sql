...
FROM (
        J1_TBL
        CROSS JOIN J2_TBL) AS tx (ii, jj, tt, ii2, kk);
...
FROM (
        J1_TBL t1 (a, b, c)
        CROSS JOIN J2_TBL t2 (d, e)) AS tx (ii, jj, tt, ii2, kk);
...
FROM (
    ...
    LEFT JOIN x xx (xx1, xx2) ON (x1 = xx1);
        ...
        FROM (
            ...
            LEFT JOIN (
                SELECT
                    ...
                    1
                FROM
                    onerow) v2 (y1, y2) ON v1.x1 = v2.y2) subq1 ON (i1.f1 = subq1.x2) ON (t1.unique2 = subq1.d1)
...
FROM
    ...
    LEFT JOIN (
        ...
        FROM (
            ...
            LEFT JOIN (
                VALUES
                    (3, 1)) v2 (y1, y2) ON v1.x1 = v2.y2) subq1 ON (i1.f1 = subq1.x2) ON (t1.unique2 = subq1.d1)
    LEFT JOIN tenk1 t2 ON (subq1.y1 = t2.unique1)
WHERE
    t1.unique2 < 42
    AND t1.stringu1 > t2.stringu2;
...
FROM
    ...
    JOIN LATERAL (
        VALUES
            (a.unique1)) ss (x) ON b.unique2 = ss.x;
...
CREATE TABLE join_pt1p1 partition of join_pt1 FOR
VALUES
FROM
    (0) TO (100)
PARTITION BY
...
CREATE TABLE join_pt1p2 partition of join_pt1 FOR
VALUES
FROM
    (100) TO (200);

CREATE TABLE join_pt1p1p1 partition of join_pt1p1 FOR
VALUES
FROM
    (0) TO (100);
