...
CREATE FUNCTION log_rewrite () RETURNS event_trigger LANGUAGE plpgsql AS $func$
...
SET DEFAULT '1970-12-31 11:12:13',
ALTER COLUMN c_timestamp_null
SET DEFAULT '2016-09-29 12:00:00';
...
ALTER COLUMN c_small
SET DEFAULT 9,
ALTER COLUMN c_small_null
SET DEFAULT 13;
...
ALTER COLUMN c_interval
DROP DEFAULT,
ALTER COLUMN c_hugetext
SET DEFAULT repeat('poiuyt', 1000);
...
ALTER TABLE T
ADD COLUMN c_array TEXT[] DEFAULT (
    '{"This", "is", "' || foo (4) || '","the", "real", "world"}')::TEXT[],
ALTER COLUMN c_timestamp
...
DROP DEFAULT,
ALTER COLUMN c_array
SET DEFAULT ('{"This", "is", "' || foo (1) || '", "fantasy"}')::TEXT[];
    ...
    stddev (
        cast ( (
                SELECT
                    sum(1)
                FROM
                    generate_series(1, 20) x) AS float4)) OVER (
        PARTITION BY
            a,
            b,
            c
...
CREATE FUNCTION test_trigger () RETURNS trigger LANGUAGE plpgsql AS $$
...
ALTER TABLE vtype2
ALTER COLUMN b TYPE varchar(20) USING b::varchar(20);
