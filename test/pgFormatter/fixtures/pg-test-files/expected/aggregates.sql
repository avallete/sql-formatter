...
SELECT
    float8_regr_accum (
        '{4,140,2900,1290,83075,15050}'::float8[],
        200,
        100);
...
SELECT
    float8_regr_combine (
        '{3,60,200,750,20000,2000}'::float8[],
        '{0,0,0,0,0,0}'::float8[]);
...
SELECT
    percentile_disc(ARRAY[0, 0.1, 0.25, 0.5, 0.75, 0.9, 1]) WITHIN GROUP (
        ORDER BY
            thousand)
FROM
    tenk1;
...
SELECT
    percentile_cont(ARRAY[0, 0.25, 0.5, 0.75, 1]) WITHIN GROUP (
        ORDER BY
            thousand)
FROM
    tenk1;
...
SELECT
    percentile_disc(ARRAY[[NULL, 1, 0.5], [0.75, 0.25, NULL]]) WITHIN GROUP (
        ORDER BY
            thousand)
FROM
    tenk1;
...
FROM
    unnest (
        '{fred,jim,fred,jack,jill,fred,jill,jim,jim,sheila,jim,sheila}'::TEXT[]) u (x);
...
SELECT
    test_percentile_disc (0.5) WITHIN GROUP (
        ORDER BY
            thousand)
FROM
    tenk1;
...
SELECT
    array (
        SELECT
            percentile_disc(a) WITHIN GROUP (
                ORDER BY
                    x)
        FROM (
                VALUES
...
SELECT
    rank('adam'::text COLLATE "C") WITHIN GROUP (
        ORDER BY
            x COLLATE "POSIX")
FROM (
        VALUES
...
SELECT
    rank('adam'::varchar) WITHIN GROUP (
        ORDER BY
            x)
FROM (
        VALUES
...
CREATE FUNCTION balkifnull (int8, int4) RETURNS int8 STRICT LANGUAGE plpgsql AS $$
BEGIN
    IF $1 IS NULL THEN
...
CREATE FUNCTION balkifnull (int8, int8) RETURNS int8 PARALLEL SAFE STRICT LANGUAGE plpgsql AS $$
BEGIN
    IF $1 IS NULL THEN
       RAISE 'erroneously called with NULL argument';
    END IF;
...
SET
    LOCAL max_parallel_workers_per_gather = 4;
