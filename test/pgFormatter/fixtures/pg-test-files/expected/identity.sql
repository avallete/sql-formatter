...
SELECT
    pg_get_serial_sequence('itest1', 'a');
...
CREATE TABLE itest_err_2 (
    a int GENERATED ALWAYS AS IDENTITY GENERATED BY DEFAULT AS IDENTITY);
...
CREATE VIEW itestv10 AS
SELECT
    *
FROM
    itest10;

CREATE VIEW itestv11 AS
SELECT
    *
FROM
    itest11;
...
INSERT INTO
    itestv10 OVERRIDING USER VALUE
VALUES
    (11, 'xyz');
...
FROM
    pg_sequence
WHERE
    seqrelid = 'itest3_a_seq'::regclass;
...
ALTER TABLE itest7d
ADD COLUMN b int GENERATED ALWAYS AS IDENTITY;
...
SELECT
,
    INSERT ON itest8 TO regress_identity_user1;
...
CREATE TABLE itest12 OF itest_type (
    f1
    WITH
        OPTIONS GENERATED ALWAYS AS IDENTITY);
...
CREATE TABLE itest_child PARTITION OF itest_parent (
    f3
    WITH
        OPTIONS GENERATED ALWAYS AS IDENTITY) FOR
VALUES
FROM
