...
FROM
    pg_proc
WHERE
    oid IN ('functest_E_1'::regproc, 'functest_E_2'::regproc)
ORDER BY
    proname;
...
SELECT
    pg_get_functiondef('functest_A_1'::regproc);

SELECT
    pg_get_functiondef('functest_B_3'::regproc);

SELECT
    pg_get_functiondef('functest_C_3'::regproc);
...
FROM
    ...
    JOIN information_schema.routines USING (specific_schema, specific_name)
WHERE
    routine_schema = 'temp_func_test'
    AND routine_name ~ '^functest_is_'
ORDER BY
    1,
    2;
