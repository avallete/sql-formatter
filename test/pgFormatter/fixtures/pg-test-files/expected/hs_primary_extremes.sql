...
CREATE OR REPLACE FUNCTION hs_subxids (n integer) RETURNS void LANGUAGE plpgsql AS $$
    BEGIN
...
CREATE OR REPLACE FUNCTION hs_locks_create (n integer) RETURNS void LANGUAGE plpgsql AS $$
    BEGIN
      IF n <= 0 THEN
		CHECKPOINT;
		RETURN;
...
CREATE OR REPLACE FUNCTION hs_locks_drop (n integer) RETURNS void LANGUAGE plpgsql AS $$
    BEGIN
      IF n <= 0 THEN
		CHECKPOINT;
		RETURN;
