...
VALUES
    (0, 'Crowberry')
ON CONFLICT (key, fruit) DO NOTHING;
...
VALUES
    (0, 'Crowberry')
ON CONFLICT (lower(fruit), key, lower(fruit), key) DO NOTHING;
...
VALUES
    (0, 'Crowberry')
ON CONFLICT (key, fruit) DO UPDATE
SET
...
VALUES
    (0, 'Crowberry')
ON CONFLICT (key, fruit text_pattern_ops) DO NOTHING;
...
VALUES
    (0, 'Crowberry')
ON CONFLICT (fruit COLLATE "C" text_pattern_ops, key) DO NOTHING;
...
VALUES
    (0, 'Crowberry')
ON CONFLICT (lower(fruit) COLLATE "C", key, key) DO NOTHING;
...
VALUES
    (0, 'Crowberry')
ON CONFLICT (fruit, key, fruit text_pattern_ops, key) DO NOTHING;
...
--
CREATE UNIQUE INDEX cross_match ON insertconflicttest (
...
VALUES
    (2, 'Orange')
ON CONFLICT (key, key, key) DO UPDATE
SET
    fruit = excluded.fruit;
...
VALUES
    (4, 'Mango')
ON CONFLICT (fruit, key) DO UPDATE
SET
    fruit = excluded.fruit;
...
VALUES
    (6, 'Passionfruit')
ON CONFLICT (lower(fruit)) DO UPDATE
SET
    fruit = excluded.fruit;

-- Check the target relation can be aliased
INSERT INTO
    insertconflicttest AS ict
VALUES
    (6, 'Passionfruit')
ON CONFLICT (key) DO UPDATE
SET
...
VALUES
    (11, 'Cherry')
ON CONFLICT (key, lower(fruit)) DO UPDATE
SET
    fruit = excluded.fruit;
...
VALUES
    (12, 'Date')
ON CONFLICT (lower(fruit), key) DO UPDATE
SET
    fruit = excluded.fruit;
...
VALUES
    (21, 'Pomegranate')
ON CONFLICT (lower(fruit), lower(fruit)) DO UPDATE
SET
    fruit = excluded.fruit;
...
VALUES
    (22, 'Apricot')
ON CONFLICT (upper(fruit)) DO UPDATE
SET
    fruit = excluded.fruit;
...
VALUES
    (26, 'Fig')
ON CONFLICT (lower(fruit), key, lower(fruit), key) DO UPDATE
SET
    fruit = excluded.fruit;
...
VALUES
    (27, 'Prune')
ON CONFLICT (key, upper(fruit)) DO UPDATE
SET
    fruit = excluded.fruit;
...
WHERE
    fruit LIKE '%berry'
    AND fruit = 'inconsequential' DO NOTHING;
...
WHERE
    fruit LIKE '%berry'
    OR fruit = 'consequential' DO NOTHING;
...
INSERT INTO
    dropcol (key, drop1, keep1, drop2, keep2)
VALUES
    (1, 1, '1', '1', 1);
...
CREATE TABLE parted_conflict_test (a int UNIQUE, b char)
PARTITION BY
    list (a);

CREATE TABLE parted_conflict_test_1 partition of parted_conflict_test (b UNIQUE) FOR
VALUES
    IN (1, 2);
...
-- partitioning level
CREATE TABLE parted_conflict (a int PRIMARY KEY, b text)
PARTITION BY
    range (a);

CREATE TABLE parted_conflict_1 partition of parted_conflict FOR
VALUES
FROM
    (0) TO (1000)
PARTITION BY
...
CREATE TABLE parted_conflict_1_1 partition of parted_conflict_1 FOR
VALUES
FROM
    (0) TO (500);
...
-- partition
CREATE TABLE parted_conflict (a int, b text)
PARTITION BY
    range (a);
...
CREATE TABLE parted_conflict (a int, b text, c int)
PARTITION BY
    range (a);
...
ALTER TABLE parted_conflict attach partition parted_conflict_1 FOR
VALUES
FROM
    (0) TO (1000);
...
AFTER
UPDATE ON parted_conflict referencing new TABLE AS inserted FOR each statement
